...
export default function LeaderboardPage() {
  ...
  return (
    <Shell>
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-1">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl text-xl" style={{ background: "rgba(0,207,255,0.12)", border: "1px solid rgba(0,207,255,0.25)" }}>🏆</div>
            <div>
              <h1 className="text-2xl font-black tracking-tight" style={{ color: "#e2f4ff" }}>Leaderboard</h1>
      ...
      <div className="mb-4 rounded-2xl p-4 backdrop-blur-sm" style={{ border: "1px solid rgba(0,207,255,0.18)", background: "rgba(6,43,69,0.75)" }}>
          ...
          <div className="flex flex-wrap gap-1.5">
            {(["cr", "wins", "kills", "mvp_count", "matches"] as SortKey[]).map((key) => (
              <button
                ...
                style={sortKey === key
                  ? { borderColor: "rgba(0,207,255,0.45)", background: "rgba(0,207,255,0.15)", color: "#00CFFF", boxShadow: "0 0 16px rgba(0,207,255,0.2)" }
                  : { borderColor: "rgba(0,207,255,0.15)", background: "rgba(0,207,255,0.05)", color: "rgba(168,255,246,0.6)" }}
              >
        ...
        <div className="mt-4 pt-4 flex flex-wrap items-end gap-4" style={{ borderTop: "1px solid rgba(0,207,255,0.15)" }}>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: "rgba(168,255,246,0.5)" }}>Rank Tier</p>
            <select
              value={rankFilter}
              ...
              style={{ border: "1px solid rgba(0,207,255,0.22)", background: "rgba(6,43,69,0.8)", color: "#e2f4ff" }}
            >
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: "rgba(168,255,246,0.5)" }}>Win Rate</p>
            <select
              value={winRateFilter}
          ...
          {(rankFilter !== "all" || winRateFilter !== "all") && (
            <button
              ...
              style={{ border: "1px solid rgba(255,127,80,0.3)", background: "rgba(255,127,80,0.08)", color: "#FF7F50" }}
            >
              ✕ Clear filters
            </button>
          )}
      ...
      {loading ? (
        ...
        <>
          <div className="overflow-hidden rounded-2xl backdrop-blur-sm" style={{ border: "1px solid rgba(0,207,255,0.18)", background: "rgba(6,43,69,0.75)" }}>
            {/* Column headers */}
            <div className="hidden md:grid grid-cols-[56px_1fr_160px_100px_130px_80px] items-center px-6 py-3 text-[10px] font-bold uppercase tracking-widest" style={{ borderBottom: "1px solid rgba(0,207,255,0.15)", background: "linear-gradient(90deg, rgba(0,207,255,0.06), transparent)", color: "rgba(168,255,246,0.5)" }}>
              <span>#</span>
              <span>Player</span>
              <span>Rank</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="hidden md:flex items-center gap-1.5 text-xs">
                      ...
                      <span className="rounded-md px-1.5 py-0.5 font-medium" style={{ background: "rgba(0,207,255,0.08)", color: "rgba(168,255,246,0.6)" }}>{winRate}%</span>
                    </div>
                    <span className="hidden md:block text-xs font-bold transition-colors" style={{ color: "rgba(168,255,246,0.5)" }}>{(p.kills || 0).toLocaleString()}</span>
                  </SoundLink>
                );
              })
            )}
