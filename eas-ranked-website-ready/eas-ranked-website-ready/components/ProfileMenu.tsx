...
export default function ProfileMenu() {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2 py-1.5 text-xs font-bold text-zinc-500 hover:bg-white/10 hover:text-white transition-colors"
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-64 rounded-xl border border-white/[0.08] bg-[#0d0d14] shadow-2xl z-50 overflow-hidden animate-fade-in">
          <div className="p-2.5 border-b border-white/[0.07]">
            <input
              ...
              className="w-full rounded-lg border border-white/10 bg-white/5 px-2.5 py-1.5 text-xs text-white placeholder-zinc-600 focus:border-orange-600/50 focus:outline-none"
            />
          </div>
          ...
          <div className="max-h-56 overflow-y-auto">
            ...
            {!loading && !query.trim() && (
              <div className="p-1.5">
                <p className="px-2 py-1 text-[10px] text-zinc-700 uppercase tracking-wider font-bold">Quick links</p>
                <button
              ...
              >
                {player.avatar_url ? (
                  ...
                  <img
                    ...
                    alt={player.name}
                    loading="lazy"
                    className="h-7 w-7 rounded-full border border-white/10 shrink-0 object-cover"
                  />
                ) : (
                  <div className="h-7 w-7 rounded-full border border-white/10 bg-white/10 flex items-center justify-center text-xs shrink-0">
                    👤
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-white truncate leading-tight">{player.name}</p>
                  {player.username && (
                    <p className="text-[10px] text-zinc-600 truncate">@{player.username}</p>
                  )}
                </div>
                <span className="text-[10px] font-black text-orange-400 shrink-0">{player.cr} CR</span>
              </button>
            ))}
          </div>
        </div>
