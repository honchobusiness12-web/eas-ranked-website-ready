...
// ---------------------------------------------------------------------------
// MatchPagination — paginated match history with smooth transitions
// ---------------------------------------------------------------------------

const MatchPagination = memo(function MatchPagination({
  points,
  itemsPerPage = 5,
  ...
  const handleExpand = useCallback(
    (idx: number) => setExpanded((prev) => (prev === idx ? null : idx)),
    []
  );
    ...
    <div>
      ...
      >
        ...
        {pageItems.map((point, idx) => {
          ...
          return (
            <div key={`${page}-${idx}`} className="relative pl-10">
              {/* Timeline dot */}
              <div
      ...
      {totalPages > 1 && (
        <div className="mt-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            disabled={page === 0}
            className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-xs font-bold text-zinc-400 transition-all duration-200 hover:bg-white/[0.08] hover:text-zinc-200 disabled:cursor-not-allowed disabled:opacity-30"
          >
            ← Back
          </button>
          ...
          <div className="flex items-center gap-1">
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                key={i}
                onClick={() => goToPage(i)}
                className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs font-bold transition-all duration-200 ${
                  i === page
                    ? "bg-purple-500/20 border border-purple-500/40 text-purple-300"
                    : "border border-white/[0.06] bg-white/[0.03] text-zinc-600 hover:bg-white/[0.07] hover:text-zinc-300"
                }`}
              >
                {i + 1}
              </button>
